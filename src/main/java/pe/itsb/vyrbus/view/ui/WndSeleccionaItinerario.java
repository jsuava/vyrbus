...
public class WndSeleccionaItinerario extends WndBase implements Serializable{
	...
	public Combobox cmbEmpresa = new Combobox();
...
//		cmdCancelar.setStyle("cursor:pointer");
		...
		this.setTitle(Messages.getString("SELECCI�N DE ITINERARIOS"));
		...
		cmbEmpresa.addEventListener(Events.ON_CHANGE, new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
				if(cmbEmpresa.getSelectedIndex()>0)
					onLoadServicio(cmbEmpresa.getSelectedItem().getValue(), cmbServicio);
				else {
					UtilData.cargarDataCombo(cmbServicio, Servicio.class, true);
				}
			}
...
//		cmbDestino.setDisabled(true);
	...
	private void buscar() throws Exception{
		String origen=""; String destino =""; String servicio = ""; String tipoDeItinerario=""; String empresa = "";
		String criterioOrden="di.d_fecpar, to_date(di.c_horpar,'HH24:MI'), di.d_feclle, to_date(di.c_horlle,'HH24:MI')";

		if (cmbEmpresa.getSelectedIndex()>0)
			empresa=((Empresa) cmbEmpresa.getSelectedItem().getValue()).getNumeroDocumento();
		...
		ListaItinerarios(ServiceLocator.getItinerarioManager().buscarItinerariosMantenimiento(null, origen,
				destino, Constantes.FORMAT_DATE.format(dbFechaInicio.getValue()), Constantes.FORMAT_DATE.format(dbFechaFin.getValue()),
				servicio,tipoDeItinerario, empresa, criterioOrden));
	}
	
	public void onLoadServicio(Empresa empresa, Combobox cmbComboBox) {
		try {
			if(empresa != null) {
				...
				UtilData.cargarDataCombo(cmbComboBox, Servicio.class, criteriosBusqueda, true);
