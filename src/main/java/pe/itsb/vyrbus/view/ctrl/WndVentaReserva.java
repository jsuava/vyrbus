 ...
 * Descripci�n	: Clase que administra todo lo relacionado a la venta de pasajes.
...
import pe.itsb.vyrbus.model.bean.Menu;
...
import pe.itsb.vyrbus.model.bean.TarifaClienteDetalle;
...
import pe.itsb.vyrbus.service.exceptions.FechaMenorCalendarioException;
import pe.itsb.vyrbus.service.exceptions.FechaNacimientoException;
import pe.itsb.vyrbus.service.exceptions.FechaViajeNoValidaException;
...
import pe.itsb.vyrbus.service.exceptions.LineaCreditoClienteException;
...
import pe.itsb.vyrbus.service.exceptions.NumeroOperacionBancariaNullException;
import pe.itsb.vyrbus.service.exceptions.NumeroOperacionTarjetaNullException;
import pe.itsb.vyrbus.service.exceptions.OpenDateOnCreditNotAllowedException;
import pe.itsb.vyrbus.service.exceptions.OperadorTarjetaCreditoNullException;
import pe.itsb.vyrbus.service.exceptions.PasajeroException;
import pe.itsb.vyrbus.service.exceptions.PasajeroIndeseableException;
import pe.itsb.vyrbus.service.exceptions.PasajeroNoSavedExeption;
...
import pe.itsb.vyrbus.service.exceptions.UsuarioNullException;
import pe.itsb.vyrbus.service.exceptions.VentaReservaException;
...
import pe.itsb.vyrbus.service.mappers.DetalleCalculadora;
...
import pe.itsb.vyrbus.service.util.AplicarPromocion;
...
import pe.itsb.vyrbus.view.ui.WndVerMapaBus;
/**
 *
...
//	private PagoSoles pagoSoles=null;
	...
	private ArrayList<DetalleCalculadora> lstDetalleCalculadora = new ArrayList<>();
	private boolean convertirPaxFre = false;
	/**
	 * Para almacenar los vouchers de las agencias de viaje
	 */
	private List<VentaPasaje> lstVentasVoucher = new ArrayList<>();
...
//		cmbEstadoCivil = (Combobox)this.getFellow("cmbEstadoCivil");
		...
		txtSearchPax = (Textbox)this.getFellow("txtSearchPax");
		/*	TAB CLIENTE	*/
...
//		tlbbtnGuardarClient = (Toolbarbutton) this.getFellow("tlbbtnGuardarClient");
		...
		txtSearchClient = (Textbox)this.getFellow("txtSearchClient");
		/*	TAB PAGOS	*/
...
//			@Override
//			public void onEvent(Event e){
//				onSearchPax(((Textbox)e.getTarget()).getText().trim().toUpperCase());
...
//		});
	...
	/**
	 * Realiza el llenado del combo que tiene la informaci�n sobre el tipo de Operacion.
	 */
...
//			especieValorada=UtilData.buscarEspecieValorada(Constantes.ID_TIPCOM_VOUCHER_CORPORATIVO, agencia,false);
	...
	/**
	 * Para habilitar o deshabilitar controles segun el tipo de operaci�n VENTA o RESERVA
	 */
	...
	public void onBuscarItinerarios(){
		try{
			...
			else if(rdVentaIdaVuelta.isChecked() && Util.comparaFechas(cldrFechaRetorno.getValue(), cldrFechaPartida.getValue(), Util.OPER_MENOR_IGUAL))
			...
			if(cmbEmpresa.getSelectedItem().getValue() instanceof Empresa)
				empresaRUC = ((Empresa)cmbEmpresa.getSelectedItem().getValue()).getNumeroDocumento();

			/*	Si no se trata de una venta a fecha abierta	*/
			if(!rdVentaFechaAbierta.isSelected()){
				/*Consulta itinerarios - TEPSA*/
				List<DetalleItinerario> lstItinerarios = ServiceLocator.getItinerarioManager().buscarItinerarios(fechaPartida, origen, destino, empresaRUC);
...
//								item.appendChild(cellTarifaSuite);//Tarifa Suite
							...
							cell = new Listcell();
							Image imgVerMapa = new Image("resources/mp_vermapa.png");
							imgVerMapa.setTooltiptext("Ver Mapa de ventas");
							cell.appendChild(imgVerMapa);
							imgVerMapa.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
								@Override
								public void onEvent(Event e) throws Exception {
									DetalleItinerario detalle = ((Listitem)e.getTarget().getParent().getParent()).getValue();
									enlazarVerMapa(detalle.getItinerario().getId());
								}
							});
							item.appendChild(cell);
				...
				if(rdVentaIdaVuelta.isSelected()){
					...
					lstItinerarios = ServiceLocator.getItinerarioManager().buscarItinerarios(fechaRetorno, origen, destino, empresaRUC);
					if(lstItinerarios.size()>0){
						Listitem item = null;
						Listcell cell = null;
...
//					throw new LiquidacionNullException();
				...
				if(chkVentaRemota.isChecked()){
					if(rdVentaIdaVuelta.isChecked() && !rdElectronicoRemoto.isChecked()){
						DlgMessage.information("No es posible realizar una venta remota de Ida y Vuelta, cuando los comprobantes a emitir no son Electr�nicos.");
						return;
					}
					...
					else if (!rdPrepagadoRemoto.isChecked() && !rdBoletoRemoto.isChecked() && !rdElectronicoRemoto.isChecked())
...
//				}
	 ...
	 * @return <b>true</b> si la validaci�n es satisfactoria, <b>false</b> caso contrario.
	 */
	private boolean validarCantidadAsientosSeleccionados(){
		boolean continuar = true;
		Map<Integer, Integer> mapSeleccionados = new HashMap<>();
		for(Listitem item : lbxAsientos.getItems()){
			DetalleItinerario detalle = (DetalleItinerario)item.getValue();
							...
							if(element3.equals(detalleItinerario.getItinerario().getServicio().getId().toString())){
								if(asientos.length==1 && asientos[0].equals("*"))
									...
									for (String element4 : asientos) {
										if(element4.equals(detalleItinerario.getAsiento().toString())){
											promo = element;
											break;
										}
									}
...
//					lblTarifa.setValue(Util.toNumberFormat((tipoCambio!=null?tipoCambio.getEquivalenteMonedaLocal():promocionAplicada.getPorImporte()),2));
					...
					if(promocionTarifa.getEsAcumulable().intValue()!=Constantes.TRUE_VALUE)
						imgPromocion.setVisible(false);
					lblPromocion.setValue("Promoci�n : "+promocionTarifa.getDenominacion());
				}
			}
...
//	}
	...
	public void validarPax() {
		try {
			disabledControlsPax(false);
			String patron = txtSearchPax.getText().trim();
			oPasajero=null;
			TreeMap<String, Object> criterioBusqueda = new TreeMap<>();
			ArrayList<Pasajero> lstPasajeros = null;
			if(patron.length() > 5) {
				...
				if(lstPasajeros != null && lstPasajeros.size()>0)
					listarRegistrosPax(lstPasajeros);
				else {
					onNewPax();
					if(Util.isNumeric(patron)) {
						...
						if(dni!=null){
							Util.seleccionarValorItemCombo(TipoDocumento.class, cmbTipoDocumento, Constantes.ID_TIPDOC_DNI);
							txtDocumentoPax.setText(dni.get(0));
							txtNombres.setText(dni.get(1));
							txtApePat.setText(dni.get(2));
							txtApeMat.setText(dni.get(3));
							txtTelefono.setFocus(true);
						}else {
							//onCleanControlsPax();
							txtDocumentoPax.setText(txtSearchPax.getText());							
						}
					}
				}
			}
...
//				throw new EmailNullException();
			...
			if(cmbTipoDocumento.getSelectedItem().getValue() instanceof TipoDocumento &&
				...
				else if (!Util.isNumeric(txtDocumentoPax.getText().trim()))
...
//			}
			...
			if(txtApePat.getText().trim().indexOf("'")>=0){
				DlgMessage.information(Messages.getString("WndVentaReserva.information.noComillaSimple")+", revise el Apellido Parteno del Pasajero.",txtApePat);
				return false;
			}else if(txtApeMat.getText().trim().indexOf("'")>=0){
				DlgMessage.information(Messages.getString("WndVentaReserva.information.noComillaSimple")+", revise el Apellido Parteno del Materno.",txtApeMat);
				return false;
			}else if(txtNombres.getText().trim().indexOf("'")>0){
				DlgMessage.information(Messages.getString("WndVentaReserva.information.noComillaSimple")+", revise los Nombres del Pasajero.",txtNombres);
				return false;
			}
...
//			quitarPromocion();
	...
	public void validarClient() {
		try {
			disabledControlsClient(false);
			String patron = txtSearchClient.getText().trim();
			oCliente=null;
			TreeMap<String, Object> criterioBusqueda = new TreeMap<>();
				...
				if(lstClientes != null && lstClientes.size()>0)
					...
					if(Util.isNumericLong(patron)) {
						...
						if(ruc!=null){
							...
							txtRazonSocial.setValue(ruc.get(1));
							txtDireccionCliente.setValue(ruc.get(2));
							txtDireccionCliente.setFocus(true);
						}else {
							//onCleanControlsPax();
							txtDocumentoCliente.setText(txtSearchClient.getText());							
						}
					}
				}
			}
...
//				throw new CantidadTrabajadoresNullException();
			...
			if(txtRazonSocial.getText().trim().indexOf("'")>=0){
				DlgMessage.information(Messages.getString("WndVentaReserva.information.noComillaSimple")+", revice la Raz�n Social del Cliente.",txtRazonSocial);
				return false;
			}else if(txtDireccionCliente.getText().trim().indexOf("'")>=0){
				DlgMessage.information(Messages.getString("WndVentaReserva.information.noComillaSimple")+", revice la Direcci�n del Cliente.",txtDireccionCliente);
				return false;
			}
...
//						tlbbtnGuardarClient.setDisabled(true);
	...
	/**
	 * Realiza una validaci�n del Tipo de Forma de Pago, para habilitar o deshabilitar algunos controles.
	 * @throws Exception
	 */
...
//
			...
			if(!(cmbTipoOperacion.getSelectedItem().getValue().equals(Constantes.TIPO_OPERACION_RESERVA)) && (dblImporte.getValue()==null || dblImporte.getValue()<=.00)) {
...
//					return;
				...
				if(dblTarifa.getValue()<=0.0){
					...
					DlgMessage.information(Messages.getString("WndVentaReserva.information.noDireccionCliente"),txtDireccionCliente);
				}else if(!(cmbTipoOperacion.getSelectedItem().getValue().equals(Constantes.TIPO_OPERACION_RESERVA)) && (dblImporte.getValue()==null || dblImporte.getValue()<=.00)) {
					DlgMessage.information(Messages.getString("WndVentaReserva.information.importePagadoCero"), dblImporte);
					return;
				}
...
//								}
	...
	private void guardarReserva(){
		final List<VentaPasaje> lstReservas = new LinkedList<>();
...
//			}
			...
			if(lbxAsientos.getItemCount()>0){
				...
				//Da formato a la fecha y hora de expiraci�n seleccionda por el usuario.
				...
				if(diferencia<Constantes.TIEMPO_MAXIMO_PERMITE_RESERVA)
...
//			if(cmbTipoFormaPago.getSelectedItem().getValue() instanceof TipoFormaPago && cmbTipoFormaPago.getText().equals(TipoFormaPago.TIPO_TRANSFERENCIA)){
			if(!txtOperacionBancaria.isDisabled() && txtOperacionBancaria.getText().trim().isEmpty()) {
					throw new NumeroOperacionTarjetaNullException();
			}else if(oPasajero.getIndeseable().intValue()==Constantes.TRUE_VALUE)
				throw new PasajeroIndeseableException();
			else if(cmbTipoFormaPago.getSelectedItem().getValue() instanceof TipoFormaPago
...
//			}
			...
			//##Comentodo 29/12/2022 - jabanto - se habilit� para el ingreso de la venta adelantada.
...
//					throw new Exception("El formato del N�mero de Boleto no es v�lido");
//				}
//			}
...
//					ventaPasaje.setFechaLiquidacion(fechaLiquidacion);
			...
			if(rdVentaIdaVuelta.isSelected()){
				final List<VentaPasaje> lstVentas = new ArrayList<>();
...
//										String msg = "La venta se registro correctamente, N�mero de Control : "+ventaPasaje.getNumeroControl();
...
//						                int tama�o;
//						                while ((tama�o = inputIda.read(buffer)) > 0) { //lee el fichero sunat a copiar cada 1MB
//						                	Salida.write(buffer, 0, tama�o);//Escribe el MB en el fichero destino
//						                }
//						                inputIda.close();
//						                while ((tama�o = inputRetorno.read(buffer)) > 0) {//lee el fichero transportista a copiar cada 1MB
//						                	Salida.write(buffer, 0, tama�o);//Escribe el MB en el fichero destino
//						                }
//						                 inputRetorno.close();
//						                 Salida.close();
...
//			column = new Column("Descripci�n", null, "200px");
...
//			}
	...
	private void agruparAsientos(){
		lbxAsientosIdaRetorno.getItems().clear();
		Map<String, List<DetalleItinerario>> mapAsientosIdaRetorno = new TreeMap<>();
		for(int i=0; i<lbxAsientos.getItems().size(); i++){
			DetalleItinerario detalle = (DetalleItinerario)lbxAsientos.getItems().get(i).getValue();
...
//			cmbDia.setText("");
	...
	private String generarFechaNacimiento(){
		String fechaNacimiento = calcularFechaNacimiento();

//		if(dtbxFechaNacimiento.getValue()!=null) {
//			fechaNacimiento = Constantes.FORMAT_DATE.format(dtbxFechaNacimiento.getValue());
//		}
...
//		try{
//			//Valida que el anio ingresado sea v�lido
...
//				dblImporte.setVisible(true);
	 ...
	 * @throws Exception
	 */
	public  void enlazarVerMapa(Long idItinerario) throws Exception {
		final WndVerMapaBus oWndSeleccionaItinerario = new WndVerMapaBus();
		oWndSeleccionaItinerario.setIdItinerario(idItinerario);
		oWndSeleccionaItinerario.setCantOcupados(0);
		oWndSeleccionaItinerario.setListOcupabilidad(null);
