...
//			lblTarifa.setValue(ventaPasaje.getFechaExpiracionReserva()!=null?ventaPasaje.getFechaExpiracionReserva().toString():"");
		...
		if(ventaPasaje.getPromocion()!=null){
			...
			String descPromocion="PROMOCI�N: "+promocion.getDenominacion()+" <=======> BENEFICIO : "+Util.toNumberFormat(Double.valueOf(promocion.getValorDescuento()),2)+tipoDescuento;
		...
		if(ventaPasaje.getTipoComprobante().getId().intValue()==Constantes.ID_TIPCOM_BOLETA_VENTA || ventaPasaje.getTipoComprobante().getId().intValue()==Constantes.ID_TIPCOM_FACTURA){
			...
			if(result.size()>0){
				...
				tbDoctReferencia.setLabel("Nota de Cr�dito");
		...
		//GPS del Comprobante - jabanto - 08/07/2024
		Util.limpiarListbox(lbxGps);
		if(!ventaPasaje.getTipoTransaccion().equals("2")) {
