...
//			if(prints!=null){
//				mostrarVentana("Configurar Impresi�n", "configPrinters.zul");
...
//			}
			...
			if(usuario.getTipoPassword().intValue()==Usuario.TIPPAS_ALEATORIO){
				...
				win.setHeight("210px");
				win.setWidth("260px");
				Caption caption = new Caption("Cambio de contrase�a", "resources/mp_changePassword.png");
				...
				Div div = new Div();
				div.setAlign("center");
				Label label = new Label("Es sistema ha detectado que es necesario que cambie su contrase�a.");
				...
				row = new Row();
				label = new Label("Contrase�a actual :");
				label.setStyle("font-weight:bold; font-size:10px !important;");
				row.appendChild(label);
				...
				row = new Row();
				label = new Label("Nueva contrase�a :");
				label.setStyle("font-weight:bold; font-size:10px !important;");
				row.appendChild(label);
				...
				row = new Row();
				label = new Label("Confirmar contrase�a :");
				label.setStyle("font-weight:bold; font-size:10px !important;");
				row.appendChild(label);
					...
					public void onEvent(Event e){
						try{
							...
							if(pex.getTipo().intValue()==PasswordException.PASSWORD_ACTUAL_NULL){
								DlgMessage.information("Debe de ingresar la contrase�a actual", txtPasswordActual);
							}else if(pex.getTipo().intValue()==PasswordException.PASSWORD_NUEVO_NULL){
								DlgMessage.information("Debe de ingresar la nueva contrase�a", txtPasswordNuevo);
							}else if(pex.getTipo().intValue()==PasswordException.PASSWORD_CONFIRMATION_NULL){
								DlgMessage.information("Vuelva a escribir la nueva contrase�a", txtConfirmPassword);
							}else if(pex.getTipo().intValue()==PasswordException.PASSWORD_DIFERENTES){
								DlgMessage.information("Las contrase�as son diferentes vuelva a escribir la nueva contrase�a.", txtConfirmPassword);
							}else if(pex.getTipo().intValue()==PasswordException.PASSWORD_IGUAL_LOGIN)
								DlgMessage.information("La contrase�a no puede ser igual al nombre de usuario.", txtPasswordNuevo);
						}catch(Exception ex){
