...
import pe.itsb.vyrbus.model.bean.TarifaFechaAbierta;
...
public class WndTarifario extends WndBase implements Serializable {
	...
	private Combobox cmbEmpresaFiltro;
...
//	private String ATTRIBUTE_ITINERARIO="8";
	...
	public void initComponents() {
		...
		cmbEmpresaFiltro=(Combobox)this.getFellow("cmbEmpresaFiltro");
...
//		ltbxMantoTarifario=(Listbox)this.getFellow("ltbxMantoTarifario");
	...
	public void onCreate() throws Exception {
		...
		//Para la busqueda
		UtilData.cargarDataCombo(cmbEmpresaFiltro, Empresa.class, true);
...
//		dlbxTarifaFa.setDisabled(getRol().getId().intValue()==Constantes.ID_ROL_MARKETING);
		...
		cmbEmpresaFiltro.addEventListener(Events.ON_CHANGE, new EventListener<Event>() {
			public void onEvent(Event event) throws Exception{
				if(cmbEmpresaFiltro.getSelectedIndex()>0) {
					onChangeEmpresa(cmbEmpresaFiltro.getSelectedItem().getValue(), cmbServicioBus, true);
				}else
					UtilData.cargarDataCombo(cmbServicioBus, Servicio.class,true);
			}
		});
		...
		cmbEmpresa.addEventListener(Events.ON_CHANGE, new EventListener<Event>() {
			public void onEvent(Event event) throws Exception{
				if(cmbEmpresa.getSelectedIndex()>0) {
					onChangeEmpresa(cmbEmpresa.getSelectedItem().getValue(), cmbServicio, true);
				}else
					UtilData.cargarDataCombo(cmbServicio, Servicio.class,true);
			}
		});
	...
	public void editarTarifa() throws Exception{
		...
		//Editar tarifas en Fecha Abierta
			...
			if (index >= 0){
				...
				//Servicio
				Util.seleccionarValorItemCombo(Servicio.class, cmbServicio, ((TarifaFechaAbierta) itemTarifa.getValue()).getServicio().getId());

				//Selecciona el Origen
				Util.seleccionarValorItemCombo(Localidad.class, cmbOrigen, ((TarifaFechaAbierta) itemTarifa.getValue()).getRuta().getLocalidadOrigen().getId());
				//Destino
				Util.seleccionarValorItemCombo(Localidad.class, cmbDestino, ((TarifaFechaAbierta) itemTarifa.getValue()).getRuta().getLocalidadDestino().getId());
					...
					if(canalVenta !=null && canalVenta.getId()!=null){
						for(Comboitem comboitemCanal: cmbCanal.getItems()) {
							if(comboitemCanal.getValue()!=null && comboitemCanal.getValue() instanceof CanalVenta) {
								if(((CanalVenta)comboitemCanal.getValue()).getId().intValue()==canalVenta.getId().intValue() && !comboitemCanal.isVisible()){
...
//			divNuevaTarifa.setVisible(false);
	...
	public void buscar() throws Exception{
		if(!chkTarifaFA.isChecked()){
			...
			if(cmbEmpresaFiltro.getSelectedItem().getValue() instanceof Empresa)
				idEmpresa = ((Empresa)cmbEmpresaFiltro.getSelectedItem().getValue()).getId();
			...
			List<TarifaRegular>lstTarifas = ServiceLocator.getTarifaRegularManager().listarTarifasPorServicios(idEmpresa, idCanalVenta, idServicio, idOrigen, idDestino, idTipoItinerario, strFechaInicioBus, strFechaFinBus, horaPartida, con_o_sin_tarifa);
	...
	//		List<TarifaFechaAbierta>lstTarifaFA=ServiceLocator.getTarifaFechaAbierta().buscarTarifas(idOrigen, idDestino, idServicio);
			...
			for(TarifaRegular tarifaRegular: lstTarifas){
				...
				//EMPRESA
				cell = new Listcell(tarifaRegular.getTarifa().getEmpresa().getNombreCorto());
				item.appendChild(cell);
	...
	public void onClick_btnGuardarTarifa(){
		try {
			if(!chkTarifaAbierta.isChecked()){
				...
				final Integer idEmpresa, idCanal, idServicio, idOrigen, idDestino, zona;
				Long seleccionado;
				Servicio oServicio;

				if(cmbEmpresa.getSelectedIndex() == 0) {
					DlgMessage.information("Debe seleccionar la Empresa para la Tarifa. ");
					cmbEmpresa.focus();
					return;
				}
	...
	//				return;
					...
					idEmpresa = ((Empresa)cmbEmpresa.getSelectedItem().getValue()).getId();
	...
	//				BUSCAR LAS TARIFAS DE ACUERDO A LO QUE SE VA A INGRESAR
					String tarifasYservicios = ServiceLocator.getTarifaRegularManager().buscarCantidadTarifasAReemplazar(idEmpresa, idCanal, idServicio, idOrigen, idDestino, piso, zona, fechaInicio, fechaFinal, strHoraServicio, porServicio);
					...
					int cantidadTarifas = Integer.parseInt(cantidades[0]);
					int cantidadServicios = Integer.parseInt(cantidades[1]);
...
//					}
					...
					if(cantidadTarifas == 0 && cantidadServicios > 0){
						...
						flag=2;
						strMensaje += "Se encontraron " + cantidades[0] + " Tarifa(s), este proceso reemplazara estas tarifas! \n ¿Realmente desea continuar?";
					}
							...
							try{
								if(e.getName().equals("onYes")){
									...
									if(dias>1){
										...
										criterioBusqueda.put("empresa", new Empresa(idEmpresa));
										...
										List <Tarifa> lstTarifa = ServiceLocator.getTarifaManager().buscarTarifa(idEmpresa, idCanal, idServicio, idOrigen, idDestino, pisoElegido, zona);
										//Si existe cabecera
										if(lstTarifa.size()>0){
											//Por cada dia hacer
											for (int x=0 ; x < dias; x ++){
												for(Tarifa tarifa: lstTarifa){
													...
													lstTarifaRegular = ServiceLocator.getTarifaRegularManager().buscarTarifasAReemplazar(tarifa.getEmpresa().getId(), tarifa.getCanalVenta().getId(), tarifa.getServicio().getId(), idOrigen, idDestino, tarifa.getPisoBus(), zona, strFechatarifa, strFechatarifa, tmbxHoraPartida.getText(), (chkPorServicio.isChecked() ? 1 : 0));
	//												Si hay Tarifas a reemplazar moverlas y volverlas a insertar segun lo ingresado
													if(lstTarifaRegular.size()>0){
														for(TarifaRegular otarifaRegular: lstTarifaRegular){
															...
															tarifaRegularAud.setIdEmpresa(otarifaRegular.getTarifa().getEmpresa().getId());
	...
	//												Si no hay tarifas a reemplazar hay que insertarlas
									...
									//Cuando es solamente una Día
										...
										if(lstTarifa.size()>0){
											for(Tarifa tarifa: lstTarifa){
												//Recuperamos el detalle de las cabeceras de tarifa
												List<TarifaRegular> lstTarifaRegular = ServiceLocator.getTarifaRegularManager().buscarTarifasAReemplazar(tarifa.getEmpresa().getId(), tarifa.getCanalVenta().getId(), tarifa.getServicio().getId(), idOrigen, idDestino, tarifa.getPisoBus(), zona, strFechatarifa, strFechatarifa, tmbxHoraPartida.getText(), (chkPorServicio.isChecked() ? 1 : 0));
												//Si hay Tarifas a reemplazar moverlas y volverlas a insertar segun lo ingresado
												if(lstTarifaRegular.size()>0){
													for(TarifaRegular otarifaRegular: lstTarifaRegular){
														//Mover las tarifas actuales a la nueva tabla para auditoria
...
//		 dlbxTarifa.setFocus(true);
	...
	public void onChangeEmpresa(Empresa empresa, Combobox cmbComboBox, boolean bTodos) {
		try {
			if(empresa != null) {
				...
				UtilData.cargarDataCombo(cmbComboBox, Servicio.class, criteriosBusqueda, bTodos);
			}
		}catch (Exception ex) {
			ex.printStackTrace();
		}		
