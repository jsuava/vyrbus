...
import pe.itsb.vyrbus.service.exceptions.NumeroAutorizacionSunatNullException;
...
//		UtilData.cargarDataCombo(cboTipoComprobante, TipoComprobante.class, false);
	...
	private void onSelectTipoComprobante()throws Exception{
		...
		/**A manera temporal el combo agencias solo se habilitar� para el rol Super Usuario*/
		...
		if(cboTipoComprobante.getSelectedItem().getValue() instanceof TipoComprobante){
			...
			if (!(tipoComprobante.getId().equals(Constantes.ID_TIPCOM_BOLETO_VIAJE) ||
				...
				//Genera los correlativos de manera autom�tica 10/11/2016 - jabanto
...
//				//Genera los correlativos de manera autom�tica
...
//				spCorrelativoFinal.setValue(999999999);
	...
	private void EventSaveEspeciValorada(final int acction) throws Exception{
		...
		if(!txtNumeroSerie.isDisabled()){
			if(serieEspecieValorada.getAgencia().getId().intValue()!=agencia.getId().intValue() && tipoComprobante.getId().intValue()==Constantes.ID_TIPCOM_MANIFIESTO_PAX){
				...
				DlgMessage.information("Este n�mero de Serie est� registrada en la Agencia "+serieEspecieValorada.getAgencia().toString()+", no puede continuar.");
				return;
			}else if(serieEspecieValorada.getAgencia().getId().intValue()!=agencia.getId().intValue()){
				Messagebox.show(Messages.getString("Este n�mero de Serie est� registrada en la Agencia "+serieEspecieValorada.getAgencia().toString()+
	...
	private void enviarCorreo(Agencia agenciaCorresponde) throws Exception{
		String title="N�mero de Serie utilizada en m�s de una Agencia";
...
//		}
		...
		mensaje+="Agencia a qui�n le corresponde :"+agenciaCorresponde.toString()+"\n";
