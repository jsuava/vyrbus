 ...
 * @since JDK1.6
 */
public class WndTipoVia extends WndOpcionesMantenimiento {
	...
	public void onSave(int action) throws Exception {
		try{
			...
			oTipoVia.setAbreviatura(txtNombreCorto.getText().trim().toUpperCase());
			oTipoVia.setEstadoRegistro(Constantes.VALUE_ACTIVO);

			switch (action) {
				case ACTION_NEW:
					UtilData.auditarRegistro(oTipoVia, getUsuario(), Executions.getCurrent());
					ServiceLocator.getTipoViaManager().guardar(oTipoVia);
					textboxId.setText(oTipoVia.getId().toString());
					break;

				case ACTION_MODIFY:
					UtilData.auditarRegistro(oTipoVia, true, getUsuario(), Executions.getCurrent());
					ServiceLocator.getTipoViaManager().actualizar(oTipoVia);
					break;
			}
	...
	public void onDelete(int tab) throws Exception {
		...
		ServiceLocator.getTipoViaManager().inactivar(id);
	}
	...
	private void listarRegistros(ArrayList<TipoVia> lstRegistros) {
		...
		for(int r = 0; r < lstRegistros.size(); r ++) {
			...
			lstFila.add(r + 1);
			lstFila.add(oTipoVia.getDenominacion());
			lstFila.add(oTipoVia.getAbreviatura());

			lstTipoVias.add(lstFila);
		}

		Util.llenarListbox(listboxLista, lstTipoVias, true);
	}

	private void mantenimientoRegistro(Long id) throws Exception {
		oTipoVia = ServiceLocator.getTipoViaManager().buscarPorId(id);

		textboxId.setText(oTipoVia.getId().toString());
		txtDenominacion.setText(oTipoVia.getDenominacion());
		txtNombreCorto.setText(oTipoVia.getAbreviatura());
	}
}