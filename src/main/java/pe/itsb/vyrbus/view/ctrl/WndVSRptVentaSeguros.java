...
public class WndVSRptVentaSeguros extends WndBase {
	...
	private Textbox txtDocumentoPasajero;
	private Combobox cmbUsuario;
	private Listbox lsbxAfiliaciones;
	...
	public void onCreate() throws Exception {
		...
		dtbxFechaInico.setValue(new Date());
		dtbxFechaFin.setValue(new Date());
		cargarAgencias();
//		cargarUsuarios();
		cargarTipoPax(cmbTipoPasajero, true);
		//cargarUsuarios();
	}

	/* (non-Javadoc)
	 * @see com.tepsa.vs.view.ui.WndBase#initComponents()
	 */
	@Override
	public void initComponents() {
		// TODO Auto-generated method stub
		dtbxFechaInico=(Datebox)this.getFellow("dtbxFechaInico");
		dtbxFechaFin=(Datebox)this.getFellow("dtbxFechaFin");
		cmbTipoPasajero=(Combobox)this.getFellow("cmbTipoPasajero");
		txtDocumentoPasajero=(Textbox)this.getFellow("txtDocumentoPasajero");
		cmbUsuario=(Combobox)this.getFellow("cmbUsuario");
		lsbxAfiliaciones=(Listbox)this.getFellow("lsbxAfiliaciones");
	...
	public void buscarAfiliaciones() throws Exception{
		...
		txtTotalPaxFree.setText("0.00");
		lblTotalPaxNormal.setVisible(false);
		...
		String boleto=txtNumeroBoleto.getText().trim().isEmpty()?null:txtNumeroBoleto.getText().trim();
		Integer idUsuario=null,idAgencia=null;
		String tipoPax=null;
		...
		for(VSAfiliacion afiliacion:lisAfilaiciones){
			...
			cell=new Listcell(afiliacion.getNumeroBoleto()!=null?afiliacion.getNumeroBoleto():"");
			cell.setStyle("font-size:11px !important;");
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getVsAsegurado().toString());
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getVsAsegurado().getTipoPasajero());
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getVsAsegurado().getNumeroDocumento());
			cell.setStyle("font-size:11px !important");
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getVsCiudad().getDenominacion());
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getNumeroCertificado());
			cell.setStyle("font-size:11px !important");
			item.appendChild(cell);
			cell=new Listcell(Constantes.FORMAT_DATE.format(afiliacion.getFechaVigenciaInicial()));
			cell.setStyle("font-size:11px !important");
			item.appendChild(cell);
			cell=new Listcell(Constantes.FORMAT_DATE.format(afiliacion.getFechaVigenciaFinal()));
			cell.setStyle("font-size:11px !important");
			item.appendChild(cell);
			...
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getAgencia()!=null?afiliacion.getAgencia().getNombreCorto():"");
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getUsuario().toString());
			item.appendChild(cell);
			cell=new Listcell(afiliacion.getFechaProcesoAfiliacion()!=null?Constantes.FORMAT_DATE.format(afiliacion.getFechaProcesoAfiliacion()):"");
			cell.setStyle("font-size:11px !important;");
			item.appendChild(cell);
...
//			item.appendChild(cell);
		...
		if(lisAfilaiciones.size()>0){
			if(cmbTipoPasajero.getSelectedIndex()<=0){
				lblTotalPaxNormal.setVisible(true);
				txtTotalPaxNormal.setVisible(true);
				lblTotalPaxFree.setVisible(true);
				txtTotalPaxFree.setVisible(true);
			}else if (cmbTipoPasajero.getSelectedIndex()==1){
				lblTotalPaxNormal.setVisible(true);
				txtTotalPaxNormal.setVisible(true);
				lblTotalPaxFree.setVisible(false);
				txtTotalPaxFree.setVisible(false);
	...
	/**
	 * Carga las Agencias, segun las fechas inicio y fin seleccinadas.
	 * @throws Exception
	 */
	public void cargarAgencias()throws Exception{
		Util.limpiarCombobox(cmbAgencias);
		Util.limpiarCombobox(cmbUsuario);
		UtilData.cargarGenericData(cmbUsuario, true);
		Util.limpiarListbox(lsbxAfiliaciones);
		txtTotalPaxNormal.setText("0.00");
		txtTotalPaxFree.setText("0.00");
		UtilData.cargarGenericData(cmbAgencias, true);
		List<Agencia>lstAgencias=ServiceLocator.getVentaSeguroManager().buscarAgenciasAfiliacionesByConsulta(getFechaInicio(), getFechaFin());
	...
	public void cargarUsuarios()throws Exception{
		...
		if(cmbAgencias.getSelectedIndex()>0){
			...
			List<Usuario>lstUsuario=ServiceLocator.getVentaSeguroManager().buscarUsuariosAfiliacionesByConsulta(getFechaInicio(), getFechaFin(), agencia.getId());
...
//			Util.exportarExcel(lsbxAfiliaciones, "Afiliaciones Emitidas-");
		if(lsbxAfiliaciones.getItems().size()>0){
			Session session = getDesktop().getSession();
			HttpSession httpSession = (HttpSession)session.getNativeSession();
			httpSession.setAttribute("parcialPath",Constantes.DIRECTORY_EXCEL+"DetalladoVentaSeguros.xls");
			httpSession.setAttribute("lbxVentaSeguros", lsbxAfiliaciones);
	...
	public void imprimir() throws Exception{
		try {
			if(lsbxAfiliaciones.getItems().size()>0){
				DateFormat FORMAT_DATE_TIME_24H = new SimpleDateFormat ("ddMMyyyyHHmmss");
				String nameFile=FORMAT_DATE_TIME_24H.format(new Date());
				File file= CreateDocument.creaDetalleVentaSeguros(lsbxAfiliaciones, nameFile, cmbAgencias.getText().trim(), cmbUsuario.getText().trim());
