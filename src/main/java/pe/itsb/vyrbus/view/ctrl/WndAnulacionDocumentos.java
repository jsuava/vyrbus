	...
	private void aplicar(){
		try {
			if(ltbxAnulacionComprobantes.getSelectedItems().size()==0){
				...
				DlgMessage.information("El modo Anulaci�n Masiva solamente se puede aplicar a un m�ximo de 20 Comprobantes");
				return;
	...
	private void validarLiquidacion(Liquidacion liquidacion)throws Exception{
		...
		if(btnProcesarAnulacion.isDisabled()){
			if(liquidacion!=null && liquidacion.getestadoLiquidacion().intValue()==Constantes.TRUE_VALUE){
				...
				lblAdvertencia.setValue("El Usuario Seleccionado no tiene una Liquidaci�n aperturada, esta es necesaria para continuar con el proceso de Anulaci�n.");
