...
import pe.itsb.vyrbus.model.bean.ItinerarioAgenciaLlegadaID;
...
import pe.itsb.vyrbus.service.exceptions.CapacidadServicioMayorExeption;
import pe.itsb.vyrbus.service.exceptions.FechaItinerarioNullException;
import pe.itsb.vyrbus.service.exceptions.HoraLlegadaNullException;
import pe.itsb.vyrbus.service.exceptions.HoraPartidaNullException;
...
import pe.itsb.vyrbus.service.exceptions.NoRemoverRegistroDelListBox;
import pe.itsb.vyrbus.service.exceptions.RutaNullException;
import pe.itsb.vyrbus.service.exceptions.ServicioNullException;
import pe.itsb.vyrbus.service.exceptions.TerminalDestinoNullException;
import pe.itsb.vyrbus.service.exceptions.TerminalOrigenNullException;
import pe.itsb.vyrbus.service.exceptions.TipoItinerarioNullException;
...
//	private Grid grdRutasTrifas;
	...
	/*Variables utilizadas para consultar itinerarios afectados en la actualizaci�n.*/
		...
		cmbEmpresa.addEventListener(Events.ON_CHANGE, new EventListener<Event>() {
			public void onEvent(Event event) {
				if(cmbEmpresa.getSelectedIndex()>0) {
					onLoadTipoServicio(cmbEmpresa.getSelectedItem().getValue(), cmbServicio);
				}else {
...
//		cmbTermDestino.setSelectedIndex(0);
	...
	public void onSearch() {
		...
		oWndFiltrar.addParameter("4. Fecha Inicio", Datebox.class);
		oWndFiltrar.addParameter("5. Fecha Final", Datebox.class);
		oWndFiltrar.addParameter("6. Servicio", Servicio.class);
		oWndFiltrar.addParameter("7. Nro. Itinerario", Intbox.class);
		...
		oWndFiltrar.addEventListener(pe.itsb.vyrbus.view.ui.Events.ON_FILTER, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				try{
					...
					if (oWndFiltrar.getParameterValue("3. Empresa") instanceof Empresa){
						Empresa empresa = (Empresa) oWndFiltrar.getParameterValue("3. Empresa");
						sEmpresa = empresa.getNumeroDocumento();
					}
					...
					if (oWndFiltrar.getParameterValue("6. Servicio") instanceof Servicio ){
						Servicio oservicio = (Servicio) oWndFiltrar.getParameterValue("6. Servicio");
						sServicio = oservicio.getDenominacion();
					}

					fechaInicio = (Date) oWndFiltrar.getParameterValue("4. Fecha Inicio");
					fechaFinal = (Date) oWndFiltrar.getParameterValue("5. Fecha Final");
					Integer itinerario = (Integer) oWndFiltrar.getParameterValue("7. Nro. Itinerario");
					if (itinerario !=null)
					...
					listarItinerarios(ServiceLocator.getItinerarioManager().buscarItinerariosMantenimiento(iItinerario, sOrigen,
							...
							sServicio,tipoDeItinerario, sEmpresa, criterioOrden));
...
//		CargarRutasTarifas();
		...
		listVentasNoUpdate=new ArrayList<>();
...
//		asignarImagenEliminarTerminal(lbxTerminalLlegada);
	...
	public void onSave(int action) throws Exception {
		try {
			...
			if(!validarDiferenciaHorasTramo(null))
						...
						try{
							if(actionf==ACTION_MODIFY){
								...
								if(listboxLista.getItems().size()==0){
									...
									sEmpresa = oitinerario.getEmpresa().getNumeroDocumento();
...
//							grdRutasTrifas.appendChild(Rows);
	...
	//	if (dbFechafin.getValue().getTime()>dbFechaInicio.getValue().getTime()){
			...
			if(action==ACTION_MODIFY){
				...
				lstItinerariosUpdate=new ArrayList<>();
				...
				for (int x=0 ; x < dias; x++){
					...
					if(action==ACTION_MODIFY){//Si es una actualizacion
						for(int i=x; i<lstItinerariosUpdate.size(); i++){
							...
							if(updateItinerario && fPartidaItiUpdate.equals(fPartidaCorr)){/*Actualiza el Itinerario*/
	...
	private List<VentaPasaje> buscaItinerariosActualizar() throws Exception{
		...
		for(VentaPasaje ventas: list){
			...
			if(ventas.getId()!=null){//Valida si el Itinerario tiene ventas.
				...
				for (Listitem it : lbxDetalleItinerario.getItems()) {
...
//	/**
//	 *
//	 * @param comboBox	: Objeto donde se cargan los terminales destino, seg�n la ruta seleccionada.
//	 * @throws Exception
//	 */
...
//			comboBox.setSelectedIndex(0);
	...
	/**
	 * 	Carga las rutas, seg�n el Origen seleccionado.
	 * @throws Exception
	 */
...
//		cmbTermDestino.getItems().clear();
		...
		if(cmbEmpresa.getSelectedItem().getValue() instanceof Empresa) {
			if (cmbOrigen.getSelectedItem().getValue() instanceof Localidad){
				...
				UtilData.cargarRutas(cmbRuta, false, Origen,null, idEmpresa);
...
//			}
			...
			oruta.setEmpresa(((Ruta)cmbRuta.getSelectedItem().getValue()).getEmpresa());
...
//			oagenciaLlegada.setDenominacion(((Agencia) cmbTermDestino.getSelectedItem().getValue()).getDenominacion());
			...
			odetalleItinerario.setEstadoRegistro(Constantes.VALUE_ACTIVO);
			List<ItinerarioAgenciaPartida> lstItinerarioAgenciaPartida = new ArrayList<>();
			...
			List<ItinerarioAgenciaLlegada> lstItinerarioAgenciaLlegada = new ArrayList<>();
...
//			final Agencia fAgenciaLlegada=oagenciaLlegada;
			...
			/*Realiza la validacion de los tiempos entre el origen y destino - 04/01/2017 - jabanto*/
			if(!validarDiferenciaHorasTramo(odetalleItinerario))
				return;
				...
				Messagebox.show(Messages.getString("WndItinerario.information.TramosDuplicados")+
					...
					public void onEvent(Event e) throws Exception {
						if(e.getName().equals("onYes")){
							...
							/*Valida si se trata de una edici�n de un Tramo*/
							if (esEdicionTramo){
...
//								HabilitaBotones_TerPartida(false);
								/*Carga Fecha Inicio para la Programaci�n*/
								dbFechaInicio.setText(dbFechaItinerario.getText());
								/*Carga Fecha Fin para la Programaci�n*/
...
//				/*Recupera el idRuta*/
				...
				criteriosBusqueda.put("empresa", odetalleItinerario.getRuta().getEmpresa());                       
				criteriosBusqueda.put("estadoRegistro", Constantes.VALUE_ACTIVO);
...
//					CargalistboxTerminalLlegada(oItinerarioAgenciaLlegada);
	 ...
	 * @param lisBoxDetalleItinerario	: ListBox al que se Agregar� el Itinerario
	 * @throws Exception
	 */
	...
	/**
	 * Confirmaci�n para quitar un tramo
	...
	private void asignarImagenEliminarTramo(){
		if(lbxDetalleItinerario.getItems().size()>0){
			for(int y=0; y<lbxDetalleItinerario.getItems().size();y++){
				...
				if(c.equals(y+1) && !btnGuardar.isDisabled()){
	...
	/**
	 * Para la edici�n del Tramo seleccionado
	 * @throws Exception
	 */
...
//				Util.seleccionarValorItemCombo(Agencia.class, cmbTermDestino, ((DetalleItinerario) itemDetalleItinerario.getValue()).getAgenciaLlegada().getId());			/*Selecciona le Terminal Destino*/
				...
				if (cmbTipoItinerario.getSelectedIndex()==0){
					/*Solo para la edici�n del Itinerario, despues de que esta haya sido guardado*/
...
// 	 * @param oClass			: Clase seg�n el terminal a agregar. (ItinerarioAgenciaPartida o ItinerarioAgenciaLlegada)
// 	 * @param listBoxTerminal	: ListBox en la que se cargar� el terminal
// 	 * @param cmbTerminal		: ComboBox de conde se agregar� el terminal de Partida o Llegada.
// 	 * @param tbHora			: TimeBox de donde se agregar� el Hora de Partida o Llegada
// 	 * @throws Exception
// 	 */
...
//	}
	...
	public static void cargarAgenciaOrigenDestino(Listbox listbox, Boolean todos, Integer localidad) throws Exception {
		...
		listbox.getItems().clear();
		for (Agencia oAgencia : lstAgenciaPartida) {
	...
	public void onSelectAgenciaOrigen() throws Exception {
		String terminales = "";
		lstAgenciaPartida = new ArrayList<>();
		try {
//			if(!strHoraPartida.equals(tbHoraPartida.getText())) {
//				Date horaInicial = Util.StringtoDate(dbFechaItinerario.getText()+" "+strHoraPartida, "dd/MM/yyyy HH:mm");
//				Date nuevaHora = Util.StringtoDate(dbFechaItinerario.getText()+" "+tbHoraPartida.getText(), "dd/MM/yyyy HH:mm");
...
//			}
	...
	public void onSelectAgenciaDestino() throws Exception{
		String terminales = "";
		lstAgenciaLlegada = new ArrayList<>();
		try {
			for(Listitem item : lbxTerminalDestino.getSelectedItems()) {
				terminales = terminales + ((Agencia)item.getValue()).getDenominacion() + ", ";
				if(((Timebox)item.getChildren().get(1).getChildren().get(0)).getText().equals(""))
	...
	public static void cargarTerminalPartidaLlegada(Combobox combobox, Boolean todos, Integer idLocalidad, int tipoTerminal) throws Exception{
		...
		combobox.getItems().clear();
		UtilData.cargarGenericData(combobox, todos);
		for (Agencia oAgencia : lstAgePar) {
			Comboitem oComboitem = new Comboitem();
...
//		}
	 ...
	 * @param listboxDetalleItinerario	: ListBox de donde se leer�n los Itinerarios para grabar.
	 * @throws Exception
	 */
	public void guardaItineratio(int action, Listbox listboxDetalleItinerario) throws Exception{
		...
		/*Recupera Ruta Mayor*/
		TreeMap<String, Object> busqRutaMayorID = new TreeMap<>();
		busqRutaMayorID.put("origen", origen);
		busqRutaMayorID.put("destino", destino);
		busqRutaMayorID.put("empresa", empresa);
...
//		otipoItinerario.setId(((DetalleItinerario) itemDetalleItinerario.getValue()).getItinerario().getTipoItinerario().getId());
		...
		oitinerario.setEmpresa(empresa);
	...
	public void guardaItinerarioDetalle(int action, Listbox listboxDetalleItinerario) throws Exception {
		try {
			List<DetalleItinerario>lstDetalleItinerarioOrg=new ArrayList<>();
			if (action==ACTION_MODIFY){
	...
	public void guardaAgenciaPartida(int action) throws Exception{
		...
		for (Listitem item : lbxDetalleItinerario.getItems()) {
			if (action==ACTION_MODIFY){
...
//	    }
	...
	/**
	 * Carga lista de itinerarios seg�n la busqueda.
	 *
	...
	private void listarItinerarios(List<DetalleItinerario> lstItinerarios) throws Exception{
		...
		if(lstItinerarios.size()>0){
			...
			for(DetalleItinerario detalleItinerario : lstItinerarios){
				...
				item.appendChild(cell);
				cell = new Listcell(detalleItinerario.getItinerario().getEmpresa().getRazonSocial());
				item.appendChild(cell);
				cell = new Listcell();
				Hbox hbox = new Hbox();
	...
	/**
	 * Habilita/deshabilita un tienerario para la visualizaci�n en la venta
	...
	private void manteniemientoItinerario(Long id) throws Exception{
		...
		if(lstItinerariosDetalle.size()>0){
			...
			Util.seleccionarValorItemCombo(Empresa.class, cmbEmpresa, oitinerario.getEmpresa().getId());
			onLoadTipoServicio(oitinerario.getEmpresa(), cmbServicio);
			...
			/*Validaci�n que evita la duplicidad de tramos, en el Detalle del Itinerario*/
			for(DetalleItinerario detalleItinerario : lstItinerariosDetalle){
				primerIndex += +1;
...
//					asignarImagenEliminarTerminal(lbxTerminalLlegada);
	...
	/**
	 * Validaci�n de itinerarios duplicados.
	 *
	 * @throws Exception
	 */
...
//		try{
			...
			if (action==ACTION_MODIFY){
				for (DetalleItinerario element : listTramoDuplicado) {
					if (! (oitinerario.getId().equals(element.getItinerario().getId())) )
...
//		}
	...
	private boolean validarDiferenciaHorasTramo(DetalleItinerario detalleItinerario)throws Exception{
		...
		if(detalleItinerario==null){
			...
			if(!esEdicionTramo && lbxDetalleItinerario.getItems().size()>0)
...
//		final Listbox ListBoxTerminal = listBoxTerminal;
	...
	public void cambiarHorario(final DetalleItinerario detalleItinerario) {
		...
		final Label lblEmpresa = new Label(detalleItinerario.getItinerario().getEmpresa().getRazonSocial());
		lblEmpresa.setStyle(style);
		row.appendChild(lblEmpresa);
		rows.appendChild(row);
							...
							public void onEvent(Event e) {
								try {
									if(e.getName().equals("onYes")) {
										...
										String rucEmpresa = detalleItinerario.getItinerario().getEmpresa().getNumeroDocumento();
										...
										listarItinerarios(ServiceLocator.getItinerarioManager().buscarItinerariosMantenimiento(null, lblOrigen.getValue(),
												"", lblFechaSalida.getValue(), lblFechaSalida.getValue(), "", "", rucEmpresa, criterio));
									}
								}catch(HoraPartidaNullException hpnex) {
	...
	public void crearVentanaClonacion(final DetalleItinerario detalleItinerario) {
		...
		lblEmpresa.setValue(detalleItinerario.getItinerario().getEmpresa().getRazonSocial());
		lblEmpresa.setStyle(style);
		row.appendChild(lblEmpresa);
		Textbox txtRucEmpresa = new Textbox();
		txtRucEmpresa.setValue(detalleItinerario.getItinerario().getEmpresa().getNumeroDocumento());
		txtRucEmpresa.setVisible(false);
		row.appendChild(txtRucEmpresa);
		rows.appendChild(row);
		...
		try {
			...
			row.appendChild(label);
			final Combobox cmbEmpresa = new Combobox();
			cmbEmpresa.setWidth("120px");
			cmbEmpresa.setDisabled(true);
			UtilData.cargarDataCombo(cmbEmpresa, Empresa.class, false);
			Util.seleccionarValorItemCombo(Empresa.class, cmbEmpresa,  detalleItinerario.getItinerario().getEmpresa().getId());
...
//			btnCancelar.setWidth("75px");
			...
			cmbEmpresa.addEventListener(Events.ON_CHANGE, new EventListener<Event>() {
				public void onEvent(Event e) {
					if(cmbEmpresa.getSelectedIndex() > 0 && cmbEmpresa.getSelectedItem().getValue() instanceof Empresa) {
						onLoadTipoServicio(cmbEmpresa.getSelectedItem().getValue(), cmbServicio );
					}					
				}
			});
	...
	public void onLoadTipoServicio(Empresa empresa, Combobox cmbComboBox) {
		try {
			if(empresa != null) {
				TreeMap<String, Object>criteriosBusqueda=new TreeMap<>();
				...
				UtilData.cargarDataCombo(cmbComboBox, Servicio.class, criteriosBusqueda, esEdicionTramo);
				//ServiceLocator.getServicioManager().buscarPorX(criteriosBusqueda, criteriosOrden);
				//UtilData.cargarTipo
			}
		}catch (Exception ex) {
			ex.printStackTrace();
