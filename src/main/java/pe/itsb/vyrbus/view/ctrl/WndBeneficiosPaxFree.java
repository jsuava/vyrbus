...
import pe.itsb.vyrbus.service.exceptions.AnioCumpleaniosPaxNullException;
import pe.itsb.vyrbus.service.exceptions.CancelaGrabacionException;
import pe.itsb.vyrbus.service.exceptions.CortesiaXCumpleaniosEmitidaException;
import pe.itsb.vyrbus.service.exceptions.FechaViajeFueraRangoException;
...
import pe.itsb.vyrbus.service.exceptions.NumeroAsientoFueraRangoException;
...
import pe.itsb.vyrbus.service.exceptions.PaxFreeInactivoException;
import pe.itsb.vyrbus.service.exceptions.PaxFreeNoRegistradoException;
import pe.itsb.vyrbus.service.exceptions.PreferenciaAlimentariaException;
import pe.itsb.vyrbus.service.exceptions.PuntosPaxfreeInsuficientesException;
import pe.itsb.vyrbus.service.exceptions.RangoCanjeCumpleaniosException;
import pe.itsb.vyrbus.service.exceptions.RutaNullException;
import pe.itsb.vyrbus.service.exceptions.RutaSinPuntajeException;
...
//		dbxFechaViaje.setValue(Constantes.FORMAT_DATE.parse(Constantes.FORMAT_DATE.format(new Date())));
	...
	public void onSearch() throws Exception {
		...
		oWndFiltrar.addEventListener(pe.itsb.vyrbus.view.ui.Events.ON_FILTER,
...
//	@SuppressWarnings("deprecation")
	...
	public void onSave(int action) throws Exception {
		try {
			...
			// Si la forma de pago es por Cumplea�os.
			if (tipoFormaPago.getId().equals(Constantes.ID_TIPFORPAG_CUMPLEANIOS)) {
				...
				// Valida si el pasajero tiene alg�n boleto cajeado por cumpleanios en el anio ingresado.
...
//		}
	...
	public void onChangeRutas() throws Exception {
		...
		if (cmbCiudadOrigen.getSelectedItem().getValue() instanceof Localidad && cmbTipoFormaPago.getSelectedItem().getValue() instanceof TipoFormaPago) {
			...
			if(((TipoFormaPago)cmbTipoFormaPago.getSelectedItem().getValue()).getId().equals(Constantes.ID_TIPFORPAG_PUNTOS)){
				...
				UtilData.cargarRutas(cmbRuta, false, Origen,Integer.valueOf(puntos.trim()),null);
			}else{
				UtilData.cargarRutas(cmbRuta, false, Origen, null, null);
			}
		} else
			UtilData.cargarGenericData(cmbRuta, false);
		liberarAsientos();
		...
		try {
			...
			if(cmbTipoFormaPago.getSelectedItem().getValue() instanceof TipoFormaPago){
				...
				if (tipoFormaPago.getId().equals(Constantes.ID_TIPFORPAG_CUMPLEANIOS)) {
					...
					cpViajesPax.setLabel("Historial de viajes por cumplea�os.");
	...
	/**
	 * Realiza la busqueda, seg�n los parametros elejidos por el usuario.
	 */
...
//			activarBtnEditar(false);
	...
	private void EdicionCortesia() throws Exception {
		...
		if (listboxLista.getSelectedItem().getValue() instanceof Cortesia) {
			...
			UtilData.cargarRutas(cmbRuta, false, ((Localidad)cmbCiudadOrigen.getSelectedItem().getValue()).getId(), null, ventaPasaje.getEmpresa().getId());
	...
	/**
	 * Activa o desactiva el bot�n Buscar pasajero
	 *
	 * @param activar
	 */
	...
	/**
	 * Carga los tipos de pago que guardan relaci�n con la forma de fago cortesia.
	 * @throws Exception
	 */
...
//			if(!(tipoFormaPago.getId().intValue()==Constantes.ID_TIPFORPAG_CORTESIA)){
	...
	private boolean validadFechaCumpleanios(Date fechaViaje) throws Exception{
		/*Valida que este dentro del periodo permitido(segun parametro configurable). Ejemplo 2 meses antes o despues del cumplea�os */
		...
		// El canje es despues de la fecha de cumplea�os.
		...
		} else {//El canje es antes de la fecha de cumplea�os.
...
//		txtNumeroBoleto.setText(especieValorada.toString());
		ControlEspecieValorada controlEspecieValorada=UtilData.buscarEspecieValoradaByCaja(Constantes.ID_TIPCOM_BOLETA_VENTA, getAgencia(), false, getUsuarioHardware(), null, null);
	...
	private void disabledImgRefresh(boolean estado){
		if(estado){
			...
			imgRefreshBoleto.setTooltiptext("Haga click aqui para refrescar el N�mero de Boleto.");
		}
	}
