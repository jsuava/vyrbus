...
import pe.itsb.vyrbus.service.exceptions.RolCargaNullException;
import pe.itsb.vyrbus.service.exceptions.RolNullException;
import pe.itsb.vyrbus.service.exceptions.SelectIndexNullException;
...
//	private UsuarioRol usuarioRol= null;
	private List<UsuarioRol>lstUsuarioRol=new ArrayList<>();
...
//
//			/* Valida que el usuario tenga el mail configurado  para las alertas que env�e el sistema */
...
//			transcarUsuarioPersonal.setAutorizaEntregaSinVerificarUsuario(chbxAutorizaEntregaSinVerificarUsuario.isChecked()?Constantes.TRUE_VALUE:Constantes.FALSE_VALUE);
			...
			/* Realiza la asociaci�n del rol con el usuario*/
//			if (action==ACTION_NEW){
...
//			}
			...
			if(cp.getTipo().intValue()==PasswordException.PASSWORD_NULL){
				...
				DlgMessage.information("La contrase�a no puede ser igual al login.", txtPassword);
				throw new CancelaGrabacionException();
			}
...
//		}
	...
	/**
	 * Realiza la asociaci�n entre el usuario y los roles.
	...
	private void onSave_usuarioRol(Usuario usuario,int action) throws Exception{
		...
		for(UsuarioRol usuarioRol:lstUsuarioRol){
			...
			for(Listitem itemSelect:lbxRoles.getItems()){
				...
				if(rol.getId().intValue()==usuarioRol.getRol().getId().intValue() && !itemSelect.isSelected()){
