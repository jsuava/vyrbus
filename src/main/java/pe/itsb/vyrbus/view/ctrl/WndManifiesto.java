...
import pe.itsb.vyrbus.service.exceptions.CertificadoHabilitacionBusNullException;
import pe.itsb.vyrbus.service.exceptions.CompPendientesXImprimirException;
import pe.itsb.vyrbus.service.exceptions.ManifiestoCorrelativoAgotadoException;
import pe.itsb.vyrbus.service.exceptions.ManifiestoDuplicateException;
import pe.itsb.vyrbus.service.exceptions.ManifiestoImplesoException;
...
import pe.itsb.vyrbus.view.ui.WndComprobantesPendientesXAsociarBoleto;
...
//								manifiesto.setCopiloto(lbcopiloto.getValue().toString().substring(1).trim());
								...
								for (Listitem item: ListPasajeros.getItems()){
									...
									//Agregar el tracking de la venta MAOE 23/06/2025
...
//				src = Constantes.URL_FORMATOS_DESPACHOS+Constantes.CLAVE_PAHT+"CARDES"+ itinerario.getId()+"-"+agencia.getId()+".txt";
				...
				if(!esPrevio){//Descarga el archivo para la impresion
...
//				xmlPrintLaser.setA_configPrint(xmlConfigPrint);
		...
		//Manifiesto de pasajeros
			...
			if(!esPrevio && (getUsuarioHardware().getPrintApplet()==null || getUsuarioHardware().getPrintApplet().intValue()==Constantes.FALSE_VALUE)){
				if(!isPrintLasert){
