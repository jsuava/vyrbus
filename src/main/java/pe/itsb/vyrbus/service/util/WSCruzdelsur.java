...
import cruzdelsur.ws.com.pe.ArrayOfDistribucion;
import cruzdelsur.ws.com.pe.ArrayOfGeneric;
import cruzdelsur.ws.com.pe.ArrayOfHorario;
import cruzdelsur.ws.com.pe.Distribucion;
import cruzdelsur.ws.com.pe.Generic;
import cruzdelsur.ws.com.pe.Horario;
import cruzdelsur.ws.com.pe.IServicecruzdelsur;
import cruzdelsur.ws.com.pe.Pasajero;
import cruzdelsur.ws.com.pe.ResultAnularBoleto;
import cruzdelsur.ws.com.pe.ResultBloquearAsiento;
import cruzdelsur.ws.com.pe.ResultDesbloquearAsiento;
import cruzdelsur.ws.com.pe.ResultVenta;
import cruzdelsur.ws.com.pe.Servicecruzdelsur;
...
public class WSCruzdelsur implements Serializable{
	...
	private static IServicecruzdelsur getSoap()throws Exception{
		...
		try {
			...
			Sendmail.enviaEmail(e.getMessage(), "Error GetSoap()", window);
			e.printStackTrace();
			return null;
		}
	}
	 ...
	 * @return
	 */
	public static List<DetalleItinerario> getHorarios(String fechaProgramacion, Integer localidad_IDOrigen, Integer localidad_IDDestino)throws Exception{
		List<DetalleItinerario>result= new ArrayList<>();
		try {
			/*Consulta si es una ruta compartida con Cruz del sur*/
			String localidadEmbarque=null,localidadDesembarque=null;
			List<PoolLocalidad>integracionOrigen=ServiceLocator.getPoolLocalidadManager().buscarByLocalidadID(localidad_IDOrigen);
			if(integracionOrigen.size()>0){
...
//			localidadDesembarque="AQP";
			if(localidadEmbarque!=null && localidadDesembarque!=null){
				ArrayOfHorario arrayOfHorario=getSoap().obtenerHorarios(fechaProgramacion, localidadEmbarque, localidadDesembarque);
				if(arrayOfHorario==null){
					return result;
				}
				for(Horario horario : arrayOfHorario.getHorario()){
					DetalleItinerario detalleItinerario=new DetalleItinerario();
					Agencia agenciaPartida= new Agencia();
					agenciaPartida.setNombreCorto(horario.getAgenciaEmbarque1().getValue());
					...
					ruta.setDestino(horario.getRuta().getValue().trim().split("-")[1].trim().toUpperCase());
					detalleItinerario.setRuta(ruta);
					detalleItinerario.setHoraPartida(horario.getFechaHoraEmbarque1().getValue().split(" ")[1].trim());
					...
					objectCruzdelsur.setLocalidadDesembarque(localidadDesembarque);
					detalleItinerario.setObjectCruzdelsur(objectCruzdelsur);
	...
	public static void crearEstructura(Groupbox grpbxParent, DetalleItinerario detalleItinerario, Grid gridOcupabilidad, Agencia agencia, Usuario usuario)throws Exception{
		try {
			...
			ObjectCruzdelsur objectCruzdelsur=detalleItinerario.getObjectCruzdelsur();
			Horario horario=objectCruzdelsur.getHorario();
			ArrayOfDistribucion arrayOfDistribucion=getSoap().obtenerAsientosDisponibles(horario.getCodigoRuta().getValue()
																						,horario.getServicioLlave().getValue()
																						,horario.getProgramacionLlave().getValue()
																						,Integer.valueOf(horario.getNumeroServicio().getValue())
			...
			int heightDivPiso1=0;
			int heightDivPiso2=0;
			for(Distribucion distribucion:arrayOfDistribucion.getDistribucion()){
				if(distribucion.getDisponible().intValue()!=-1){
					int Y=distribucion.getY1()+41;
					int piso=Integer.valueOf(distribucion.getNivelPiso().getValue());
					if(piso==2){
						Y+=+30;
					}
					...
					if(distribucion.getDisponible().intValue()==Constantes.TRUE_VALUE){
						...
						asiento.setDisponible(distribucion.getDisponible());
						asiento.setTarifa(distribucion.getTarifatotal());
						...
						imagen=generarImagen(AsientoPool.imagenOcupado, null, 18);
					...
					if(piso==1)
						...
						if(divMapaPriso2==null)
							divMapaPriso2= new Div();
						divMapaPriso2.appendChild(asiento);
					}

					if(heightDivPiso1<Y && piso==1)
						heightDivPiso1=Y;
					else if(heightDivPiso2<Y && piso==2)
						heightDivPiso2=Y;
				}
			}
			...
			label = new Label(String.valueOf(asientosOcupados));
			row.appendChild(label);
			label = new Label(String.valueOf(asientosDisponibles));
			row.appendChild(label);
			rows.appendChild(row);
			gridOcupabilidad.appendChild(rows);
		} catch (Exception e) {
			DlgMessage.information("El Servicio de Cruz del Sur ha retornado el Siguiente error: \n"+e.getMessage());
		}
	}
	...
	private static void onClickAsiento(Event e)throws Exception{
		...
		if(asiento.getDisponible().intValue()==Constantes.TRUE_VALUE){
			...
			ResultBloquearAsiento resultBloquearAsiento=getSoap().bloquearAsiento(horario.getCodigoRuta().getValue()
																				, objectCruzdelsur.getLocalidadEmbarque(), objectCruzdelsur.getLocalidadDesembarque()
																				, horario.getNumeroServicio().getValue()
																				, horario.getProgramacionLlave().getValue(), horario.getServicioLlave().getValue()
																				, asiento.getNumeroAsiento().toString(), asiento.getNivelAsiento().toString());
			if(resultBloquearAsiento.isIsCorrect()){
				...
				DlgMessage.information(resultBloquearAsiento.getError().getValue().getMessage().getValue());
			}
	 ...
	 * @throws Exception
	 */
	public static ResultDesbloquearAsiento desbloquearAsiento(AsientoPool asiento)throws Exception{
		ResultDesbloquearAsiento resultDesbloquearAsiento=getSoap().desbloquearAsiento(asiento.getResultBloquearAsiento().getCodigoTransaccion().getValue());
		if(resultDesbloquearAsiento.isIsCorrect()){
	...
	public static ResultVenta enviarVenta(String codigoTransaccion, String agenciaEmbarqueLlave, String agenciaDesembarqueLlave,pe.itsb.vyrbus.model.bean.Pasajero oPasajero, List<Generic> menus, String numeroBoleto)throws Exception{
	...
	public static void anularBoleto(String numeroBoleto)throws Exception{
		...
		if(resultAnularBoleto.isAnulado()){
			...
			if(resultAnularBoleto.getError().getValue()==null){
				...
				throw new Exception(resultAnularBoleto.getError().getValue().getMessage().getValue());
			}
		}
	}
