...
import mstc.ws.gob.pe.Anular;
import mstc.ws.gob.pe.ArrayOfDocumento;
import mstc.ws.gob.pe.Conductor;
import mstc.ws.gob.pe.Documento;
import mstc.ws.gob.pe.Errores;
import mstc.ws.gob.pe.Finalizar;
import mstc.ws.gob.pe.HojaRuta;
import mstc.ws.gob.pe.Manifiesto;
import mstc.ws.gob.pe.Parametro;
import mstc.ws.gob.pe.ResultAnular;
import mstc.ws.gob.pe.ResultConductor;
import mstc.ws.gob.pe.ResultFinalizar;
import mstc.ws.gob.pe.ResultHojaRuta;
import mstc.ws.gob.pe.ResultManifiesto;
import mstc.ws.gob.pe.ResultParametro;
import mstc.ws.gob.pe.ResultTripulante;
import mstc.ws.gob.pe.ResultVehiculo;
import mstc.ws.gob.pe.Seguridad;
...
import pe.itsb.vyrbus.service.exceptions.WSMTCExcepcion;
 ...
 */
public class WSMTC {
	public static final int TIPO_CONDUCTOR_PILOTO=1;
	public static final int TIPO_CONDUCTOR_COPILOTO=2;
	public static final int TIPO_CONDUCTOR_COPILOTOAUXILIAR=3;
	public static final int HORA_INICIO_NORTURNO=22;
	public static final int MINUTO_INICIO_NORTURNO=0;
	public static final int HORA_FIN_NORTURNO=6;
	public static final int TIPO_TRIPULANTE=4;
	public static final int HORAS_CONDUCCION=5; //Indica l
	public static final int HORAS_CONDUCCION_NOCTURNA=4; //Cantidad de horas de conduccion nocturna
	private static WSServiciosHRSoap soap;
	private static String MESSAGE_NULL_MTC="El MTC ha encontrado errores en la validaci�n, pero este no ha retornado ning�n Mensaje.";
	private static String MESSAGE_ERROR_PARAMETROS="Ha ocurrido un error al recuperar los parametros del MTC";
	private static ArrayOfDocumento arrayOfDocumento=null;
	private static String MESSAGE_NO_CONECTION_WEBSERVICE="No se pudo establecer conexi�n con el Servicio Web del MTC";
	private static String EXCEPTION_MESSAGE=null;

	/**
	 * @return the soap
	 */
	public static WSServiciosHRSoap getSoap() throws Exception {
		try{
			if(soap==null || EXCEPTION_MESSAGE!=null){
	 ...
	 * @param NumeroDocumento	: N�mero de Documento del Conductor.
	 * @param tipoConductor		: Tipo de Conductor, Piloto, Copiloto, tercer Piloto, etc
	 * @throws WSMTCExcepcion
	 * @throws Exception
	 */
	public static String validarConductor(String tipoDocumento,String NumeroDocumento, Integer tipoConductor) throws WSMTCExcepcion, Exception{
		try {
			...
			String message="El MTC reporta lo siguiente sobre el "+stipoConductor+", por lo que �ste se encuentra INABILITADO para conducir:\n"+wse.getMessage();
...
//			throw new WSMTCExcepcion(e.getMessage());
	 ...
	 * @return	: Retorna una clase ResultConductor
	 * @throws Exception
	 */
	private static void validarConductor(String tipoDocumento,String NumeroDocumento) throws Exception, WSMTCExcepcion {
		Conductor conductor=new Conductor();
		conductor.setTpoDocumento(tipoDocumento);
		...
		/* En el caso que el MTC reporte alg�n error*/
	...
	/**
	 * Realiza la validacion del Vehiculo con el WS del MTC
	 * @param numeroPlaca : N�mero de la plaza del Vehiculo
	 * @return Retorna una Clase ResultVehiculo
	 * @throws Exception
	 */
...
//	}
	 ...
	 * @throws Exception
	 */
	public static boolean anularHRE(String numeroHRE)throws Exception{
	 ...
	 * @throws Exception
	 */
	public static ResultTripulante setTripulante(Tripulante tripulante)throws Exception{
		tripulante.setSeguridad(getSeguridad());
	 ...
	 */
	private static Seguridad getSeguridad() {
		Seguridad seguridad=new Seguridad();
		seguridad.setRuc("20502324927");
		seguridad.setUsuario("059979");
		seguridad.setClave("xKIx5z");
	...
	private static String getMessageError(List<Errores>lstErrores){
		String messages="";
		for(Errores errores:lstErrores){
			messages+=errores.getCode()+" - "+errores.getInfo()+"\n";
		}
		return messages;
	}
	 ...
	 * @param tipo					: Indica el tipo de persona del que se va a recuperar el tipoDocumento (Pilot, copilo, tripulante, etc)
	 * @return Tipo documento seg�n MTC
	 * @throws Exception
	 */
	public static String getTipoDocumento(Integer idTipoDocumentoSisVyr, Integer tipoPersona )throws Exception, WSMTCExcepcion{
		try {
			...
			if(arrayOfDocumento==null){
				...
				parametro.setSeguridad(getSeguridad());
				ResultParametro resultParametro=getSoap().getParametro(parametro);
				if (resultParametro.isReturn())
					arrayOfDocumento=resultParametro.getDocumentos();
				else{
					String message=resultParametro.getErrores()!=null?getMessageError(resultParametro.getErrores().getErrores()):MESSAGE_NULL_MTC;
					String messages="El MTC reporta el siguiente Error al recuperar los parametros: \n"+message;
					if(permitirAlRolOmicionErres())
						return messages;
					else{
						throw new WSMTCExcepcion(messages);
			...
			if(arrayOfDocumento!=null && arrayOfDocumento.getDocumento()!=null){
				for(Documento documento: arrayOfDocumento.getDocumento()){
					if(documento.getCode().equals("L.E.") && idTipoDocumentoSisVyr.intValue()==Constantes.ID_TIPDOC_DNI){
						tipoDocumento=documento.getCode();
						break;
					}else if (documento.getCode().equals("C.E.") && idTipoDocumentoSisVyr.intValue()==Constantes.ID_TIPDOC_CARNET_EXTRANJERIA){
						tipoDocumento=documento.getCode();
						break;
					}else if (documento.getCode().equals("P.S.") && idTipoDocumentoSisVyr.intValue()==Constantes.ID_TIPDOC_PASAPORTE){
						tipoDocumento=documento.getCode();
						break;
					}
				}
			}else{
				if(permitirAlRolOmicionErres(MESSAGE_ERROR_PARAMETROS))
					return MESSAGE_ERROR_PARAMETROS;
				else
			...
			if(tipoDocumento==null){
				String message="";
				if(stipoPersona!=null)
					message="El MTC no tiene registrado el tipo de documento del "+stipoPersona+", por favor verificarlo si este esta registrado correctamente.";
				else
					message="No se encontro el tipo de documento en el MTC.";
				if(permitirAlRolOmicionErres())
					return message;
				else
					throw new WSMTCExcepcion(message);
	 ...
	 * @throws Exception
	 */
	private static String getTipoPersona(Integer tipo)throws Exception{
		String stipo="";
		switch (tipo.intValue()) {
		case TIPO_CONDUCTOR_PILOTO:
			stipo="PILOTO";
			break;
		case TIPO_CONDUCTOR_COPILOTO:
			stipo="COPILOTO";
			break;
		case TIPO_CONDUCTOR_COPILOTOAUXILIAR:
			stipo="COPILOTO AUXILIAR";
			break;
	 ...
	 */
	private static Boolean permitirAlRolOmicionErres(){
		return permitirAlRolOmicionErres(null);
	}
	/**
	 * Valida los roles a los cuales se le debe permitir amitir la validacion en caso de que la ws del MTC retorne algun error.
	 * @return true si al usuario se le permitira omitir el error emitido por el MTC
	 */
	private static Boolean permitirAlRolOmicionErres(String exceptionMessage){
		if( exceptionMessage==null){
			...
			//Validaci�n cuando el servicio web retorna una exception desconocida.
	...
	public static void enviarCorreo(String messageMtc,ProgramacionServicio programacionServicio, String usuario, String agencia){
		...
		String mensaje="Se ha omitido el siguiente error retornado por el MTC :\n Mensaje mostrado al Usuario :"+messageMtc;
		mensaje+="\n Exception retornada por el sistema :"+(EXCEPTION_MESSAGE!=null?EXCEPTION_MESSAGE:"");
		mensaje+="\n\n Itinerario :"+programacionServicio.getItinerario().getId()+"Bus :"+programacionServicio.getBus().getCodigo();
		mensaje+="\n Ruta       :"+programacionServicio.getItinerario().getRuta().toString();
		mensaje+="\n Piloto     :"+programacionServicio.getPiloto().toString();
		mensaje+="\n Copiloto  :"+programacionServicio.getCopiloto().toString();
		mensaje+="\n Usuario    :"+usuario.toString();
		mensaje+="\n Agencia    :"+agencia.toString();
		mensaje+="\n Fecha y Hora :"+Constantes.FORMAT_DATE_TIME_24H.format(new Date());
		try {
...
//	}
		...
		try {
			...
			try {
				...
				if(resultIdentidad!=null && resultIdentidad.isReturn()){
					int longitudMax=30;
					/*Valida longitud del Apellido Paterno*/
					if(resultIdentidad.getPaterno()!=null && resultIdentidad.getPaterno().trim().length()>longitudMax){
						String paterno=resultIdentidad.getPaterno().trim();
						resultIdentidad.setPaterno(paterno.substring(0, longitudMax));
					}
					/*Valida longitud del Apellido Materno*/
					if(resultIdentidad.getMaterno()!=null && resultIdentidad.getMaterno().trim().length()>longitudMax){
						String materno=resultIdentidad.getMaterno().trim();
						resultIdentidad.setMaterno(materno.substring(0, longitudMax));
					}
					/*Valida longitud del Apellido Nombres*/
					if(resultIdentidad.getNombre()!=null && resultIdentidad.getNombre().trim().length()>longitudMax){
						String nombres=resultIdentidad.getNombre().trim();
			...
			e.printStackTrace();
			try {
				inactivarValidacon(e.getMessage()!=null?e.getMessage():" ");
			} catch (Exception ex) {
				ex.printStackTrace();
			}
			return null;
	...
	private static void inactivarValidacon(String messageException)throws Exception{
		...
		/*******************************************************************************************************************/
		Parametros parametros=ServiceLocator.getParametrosManager().buscarPorEstadoRegistro(Constantes.VALUE_ACTIVO);
		/*Inactiva si esta activo*/
		if(parametros.getValidarDNIgetIdentidad()!=null && parametros.getValidarDNIgetIdentidad().intValue()==Constantes.TRUE_VALUE){
			parametros.setAlertarVentaDNIFalso(Constantes.FALSE_VALUE);
			parametros.setValidarDNIgetIdentidad(Constantes.FALSE_VALUE);
...
//			mensaje+="La reactivaci�n se realizar� de forma autom�tica, apenas se obtenga respuesta de este M�todo.\n";
	...
	/**
	 * Actualiza los datos personales del pasajero con los de la reniec.
	 * @param pasajero : Objeto pasajero.
	 * @return
	 */
	public static Pasajero updateDatosPaxByReniec(Pasajero pasajero){
		try {
			Parametros parametros=ServiceLocator.getParametrosManager().buscarPorEstadoRegistro(Constantes.VALUE_ACTIVO);
			if(parametros.getValidarDNIgetIdentidad()!=null && parametros.getValidarDNIgetIdentidad().intValue()==Constantes.TRUE_VALUE){
				/*busca los datos del pasajero en la reniec*/
				...
				if(resultIdentidad!=null && resultIdentidad.isReturn()){
					...
					if(resultIdentidad.getPaterno()!=null && !(resultIdentidad.getPaterno().trim().isEmpty()))
						...
						pasajero.setApellidoPaterno(resultIdentidad.getMaterno().trim().toUpperCase());
					//Valida el apellido materno
					if(!(resultIdentidad.getMaterno()!=null && resultIdentidad.getMaterno().trim().isEmpty()))
					...
					pasajero.setIpModificacion("192.168.200.70");
	...
	public static String setManifiesto(Manifiesto manifiesto)throws Exception{
		...
		if(!(resultManifiesto.isReturn())){
			String message="El MTC ha retornado los siguientes mensajes al Generar le Manifiesto Electr�nico. \n";
			...
			if(resultManifiesto.getCode()!=null)
				return resultManifiesto.getCode();
			else
				return "NO CODE";
		}
	}
