...
//            WritableSheet s = w.getSheet(0);
            ...
            for (Iterator it = head.getChildren().iterator(); it.hasNext();) {
				...
				rowh.createCell((short)0).setCellValue(new HSSFRichTextString(header.getLabel()));
//                s.addCell(new Label(i, j, header.getLabel()));
                i++;
            }
            j++;
...
//                        s.addCell(new Number(i, j, Integer.parseInt(currentCell.getLabel()), cellNumberFormat));
...
//                            WritableCellFormat cellNumberFormat = new WritableCellFormat(numberformat);
//                            s.addCell(new Number(i, j, Double.parseDouble(currentCell.getLabel()), cellNumberFormat));
                            cellh = rowh.createCell((short)i);
    						cellh.setCellStyle(style);
    						cellh.setCellValue(Integer.valueOf(currentCell.getLabel()));
                        } else {
                        	rowh.createCell((short) i).setCellValue(new HSSFRichTextString(currentCell.getLabel()));
//                            s.addCell(new Label(i, j, currentCell.getLabel()));
                        }
                    }
                    i++;
                }
