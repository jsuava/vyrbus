...
//
			...
			/*	Realizamos la carga de informaci�n a los controles combobox	*/
					...
					try{
						if(e.getName().equals("onYes")){
							...
							for(VentaPasaje reserva : lstReservas){
								result = ServiceLocator.getVentaPasajesManager().guardarVenta(reserva, rdVentaFechaAbierta.isSelected(), true, true, true, false);
							}
	...
	private void guardarVentaIdaVuelta(final List<VentaPasaje> lstVentas){
		Messagebox.show(Messages.getString("WndVentaReserva.information.confirmacionGuardarVenta"), DlgMessage.NOMBREAPLICACION, DlgMessage.BTN_YESNO, Messagebox.QUESTION, new EventListener<Event>() {
			...
			public void onEvent(Event e) throws Exception{
				try{
					if(e.getName().equals("onYes")){
						int result = ServiceLocator.getVentaPasajesManager().guardarVentaIdaVuelta(lstVentas, true, false);
...
//	}
	...
	private void imgPromocion_loadPromociones(){
		try{
			...
			//MAOE 22/04/2024 no ser�a necesario validar si el pasajero es nulo
//			if(oPasajero==null){
