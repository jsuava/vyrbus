 ...
 * @since JDK1.6
 */
public class WndEstadoCivil extends WndOpcionesMantenimiento {
	...
	public void onSave(int action) throws Exception {
		try {
			...
			oEstadoCivil.setDenominacion(txtDenominacion.getText().trim().toUpperCase());
			oEstadoCivil.setEstadoRegistro(Constantes.VALUE_ACTIVO);

			switch (action) {
				case ACTION_NEW:
					UtilData.auditarRegistro(oEstadoCivil, getUsuario(), Executions.getCurrent());
					ServiceLocator.getEstadoCivilManager().guardar(oEstadoCivil);
					textboxId.setText(oEstadoCivil.getId().toString());
					break;

				case ACTION_MODIFY:
					UtilData.auditarRegistro(oEstadoCivil, true, getUsuario(), Executions.getCurrent());
					ServiceLocator.getEstadoCivilManager().actualizar(oEstadoCivil);
					break;
			}
	...
	public void onDelete(int tab) throws Exception {
		...
		ServiceLocator.getEstadoCivilManager().inactivar(id);
	}
	/* (non-Javadoc)
	...
	private void listarRegistros(ArrayList<EstadoCivil> lstRegistros) {
		...
		for(int r = 0; r < lstRegistros.size(); r ++) {
			...
			lstFila.add(r + 1);
			lstFila.add(oEstadoCivil.getDenominacion());

			lstEstadosCiviles.add(lstFila);
		}

		Util.llenarListbox(listboxLista, lstEstadosCiviles, true);
	}


	private void mantenimientoRegistro(Long id) throws Exception {
		oEstadoCivil = ServiceLocator.getEstadoCivilManager().buscarPorId(id);

		textboxId.setText(oEstadoCivil.getId().toString());
		txtDenominacion.setText(oEstadoCivil.getDenominacion());
	}
}