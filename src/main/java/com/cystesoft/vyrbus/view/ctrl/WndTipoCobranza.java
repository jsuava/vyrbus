 ...
 *
 */
public class WndTipoCobranza extends WndOpcionesMantenimiento {
...
final WndFiltrarParametros oWndFiltrar = new WndFiltrarParametros();
		...
		try{
			...
			switch (action) {
				case ACTION_NEW:
					...
					ServiceLocator.getTipoCobranzaManager().guardar(tipoCobranza);
					break;
				case ACTION_MODIFY:
					UtilData.auditarRegistro(tipoCobranza, true, getUsuario(), Executions.getCurrent());
	...
	public void onDelete(int tab) throws Exception {
		...
		if(listboxLista.getSelectedIndex()>=0){
			TipoCobranza tipoCobranza=listboxLista.getSelectedItem().getValue();
			ServiceLocator.getTipoCobranzaManager().inactivar(tipoCobranza.getId().longValue());
		}
	}
