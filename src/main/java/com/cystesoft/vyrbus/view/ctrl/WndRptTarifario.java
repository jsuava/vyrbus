...
public class WndRptTarifario extends WndBase implements Serializable {
	...
	public void initComponents() {
		...
		chbxTarifaCero=(Checkbox)this.getFellow("chbxTarifaCero");
		lstbxTarifario=(Listbox) this.getFellow("lstbxTarifario");
	}
...
//					ResultIdentidad resultIdentidad=WSMTC.getIdentidad(pasajero.getNumeroDocumento());
//					String dni=pasajero.getNumeroDocumento();
//					String datosPersonales="****** DOCUMENTO NO ENCONTRADO ****** ";
...
//		}
		...
		if (lsDetalleItinerarios.size() > 0){
			...
			for (DetalleItinerario detalleItinerario : lsDetalleItinerarios){
				...
				item.appendChild(cell);
				cell= new Listcell((!idItinrario.equals(itinerario.getId()))?Constantes.FORMAT_DATE.format(itinerario.getFechaPartida())+"  "+itinerario.getHoraPartida(): ""); //Partida itinerario
			...
			for(DetalleItinerario detalleItinerario:lsDetalleItinerarios){
				...
				for(Comboitem comboitem:cmbRuta.getItems()){
					if(comboitem.getValue()!=null){
						Ruta rutaAgregada=(Ruta) comboitem.getValue();
						if(ruta.getId().intValue()==rutaAgregada.getId().intValue()){
							existeRuta=true;
							break;
						}
					}
				...
				if (!(existeRuta)){
					...
					comboitem.setLabel(ruta.toString());
					comboitem.setValue(ruta);
