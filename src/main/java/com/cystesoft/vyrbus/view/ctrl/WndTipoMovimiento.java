 ...
 * @since JDK1.6
 */
public class WndTipoMovimiento extends WndOpcionesMantenimiento {

	private static final long serialVersionUID = -1721402526478904823L;

	private Textbox txtDenominacion;
	
	private TipoMovimiento oCondicionVenta = null;
	...
	public void onSearch() {
		...
		oWndFiltrar.addEventListener(com.cystesoft.vyrbus.view.ui.Events.ON_FILTER, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				...
				listarRegistros(ServiceLocator.getTipoMovimientoManager().buscarPorX(criteriosBusqueda, criteriosOrdenar));
			}
		});
	}
	/* (non-Javadoc)
	...
	public void onRefresh(int tab) throws Exception {
		if (!criteriosBusqueda.isEmpty()) {
			this.listarRegistros(ServiceLocator.getTipoMovimientoManager().buscarPorX(criteriosBusqueda, criteriosOrdenar));					
		}
	}
	/* (non-Javadoc)
	...
	public void onSave(int action) throws Exception {
		try {
			...
			if (action==ACTION_NEW)
				oCondicionVenta = new TipoMovimiento();
			
			Integer id = (textboxId.getText().equals("") ? 0 : new Integer(textboxId.getText()));
			oCondicionVenta.setId(id);
			oCondicionVenta.setDenominacion(txtDenominacion.getText().trim().toUpperCase());
			oCondicionVenta.setEstadoRegistro(Constantes.VALUE_ACTIVO);

			switch (action) {
				case ACTION_NEW:
					UtilData.auditarRegistro(oCondicionVenta,getUsuario(), Executions.getCurrent());
					ServiceLocator.getTipoMovimientoManager().guardar(oCondicionVenta);
					textboxId.setText(oCondicionVenta.getId().toString());
					break;

				case ACTION_MODIFY:
					UtilData.auditarRegistro(oCondicionVenta, true,getUsuario(), Executions.getCurrent());
					ServiceLocator.getTipoMovimientoManager().actualizar(oCondicionVenta);
					break;
			}
			/*RECUPERA EL REGISTRO ACTUALIZADO O EL NUENO*/
			criteriosBusqueda.remove("denominacion");
			criteriosBusqueda.put("estadoRegistro", Constantes.VALUE_ACTIVO);
			criteriosBusqueda.put("denominacion", oCondicionVenta.getDenominacion());
			listarRegistros(ServiceLocator.getTipoMovimientoManager().buscarPorX(criteriosBusqueda, criteriosOrdenar));
	...
	public void onDelete(int tab) throws Exception {
		...
		ServiceLocator.getTipoMovimientoManager().inactivar(id);
	}
	/* (non-Javadoc)
	...
	private void listarRegistros(ArrayList<TipoMovimiento> lstRegistros) {
		ArrayList<Object> lstCondicionesVenta = new ArrayList<Object>();

		for(int r = 0; r < lstRegistros.size(); r ++) {
			TipoMovimiento oCondicionVenta = lstRegistros.get(r);
			ArrayList<Object> lstFila = new ArrayList<Object>();

			lstFila.add(oCondicionVenta.getId());
			lstFila.add(r + 1);
			lstFila.add(oCondicionVenta.getDenominacion());

			lstCondicionesVenta.add(lstFila);
		}

		Util.llenarListbox(listboxLista, lstCondicionesVenta, true);
	}


	private void mantenimientoRegistro(Long id) throws Exception {
		oCondicionVenta = ServiceLocator.getTipoMovimientoManager().buscarPorId(id);
		textboxId.setText(oCondicionVenta.getId().toString());
		txtDenominacion.setText(oCondicionVenta.getDenominacion());
	}
}