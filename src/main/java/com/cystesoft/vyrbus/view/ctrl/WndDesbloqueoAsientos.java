...
public class WndDesbloqueoAsientos extends WndBase implements Serializable{
	...
	public void listarRegistros()throws Exception {
		List<TmpOcupacionAsientos> lstRegistros = new ArrayList<TmpOcupacionAsientos>();
...
//				item.appendChild(cell);
				...
				if(accesoEliminar() ){
					...
					if(parametros.getTiempoExpiraBloqueoPersonalizado()!=null){
						...
						for(int i=0; i<TimeExpiBloqPers.length;i++){
							String[] agenciaTime=TimeExpiBloqPers[i].split(",");// Identificador de la agencia
							if(agenciaTime.length==2){
								Integer idAgencia=Integer.valueOf(agenciaTime[0]);
								if(tmpOcupacionAsientos.getUsuarioHardware().getAgencia().getId().intValue()==idAgencia.intValue()){//Valida Agencia.
