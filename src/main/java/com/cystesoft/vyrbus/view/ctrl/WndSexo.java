...
public class WndSexo extends WndOpcionesMantenimiento {
	...
	public void onSearch() {
		...
		oWndFiltrar.addEventListener(com.cystesoft.vyrbus.view.ui.Events.ON_FILTER, new EventListener<Event>() {
			...
			public void onEvent(Event event) throws Exception {
				...
				listarRegistros(ServiceLocator.getSexoManager().buscarPorX(criteriosBusqueda, criteriosOrdenar));
			}
		});
	}
	/* (non-Javadoc)
	...
	public void onRefresh(int tab) throws Exception {
		if (!criteriosBusqueda.isEmpty()) {
			this.listarRegistros(ServiceLocator.getSexoManager().buscarPorX(criteriosBusqueda, criteriosOrdenar));					
		}
	}
	/* (non-Javadoc)
	...
	public void onSave(int action) throws Exception {
		try{
			...
			oSexo.setId(id);
			oSexo.setDenominacion(txtDenominacion.getText().trim().toUpperCase());
			oSexo.setEstadoRegistro(Constantes.VALUE_ACTIVO);

			switch (action) {
				case ACTION_NEW:
					UtilData.auditarRegistro(oSexo, getUsuario(), Executions.getCurrent());
					ServiceLocator.getSexoManager().guardar(oSexo);
					textboxId.setText(oSexo.getId().toString());
					break;

				case ACTION_MODIFY:
					UtilData.auditarRegistro(oSexo, true, getUsuario(), Executions.getCurrent());
					ServiceLocator.getSexoManager().actualizar(oSexo);
					break;
			}
			/*RECUPERA EL REGISTRO ACTUALIZADO O EL NUEVO*/
			criteriosBusqueda.remove("denominacion");
			criteriosBusqueda.put("denominacion", oSexo.getDenominacion());
	...
	public void onDelete(int tab) throws Exception {
		...
		ServiceLocator.getSexoManager().inactivar(id);
	}
	/* (non-Javadoc)
	...
	private void listarRegistros(ArrayList<Sexo> lstRegistros) {
		ArrayList<Object> lstSexos = new ArrayList<Object>();

		for(int r = 0; r < lstRegistros.size(); r ++) {
			Sexo oSexo = lstRegistros.get(r);
			ArrayList<Object> lstFila = new ArrayList<Object>();

			lstFila.add(oSexo.getId());
			lstFila.add(r + 1);
			lstFila.add(oSexo.getDenominacion());

			lstSexos.add(lstFila);
		}

		Util.llenarListbox(listboxLista, lstSexos, true);
	}


	private void mantenimientoRegistro(Long id) throws Exception {
		oSexo = ServiceLocator.getSexoManager().buscarPorId(id);

		textboxId.setText(oSexo.getId().toString());
		txtDenominacion.setText(oSexo.getDenominacion());
	}
}