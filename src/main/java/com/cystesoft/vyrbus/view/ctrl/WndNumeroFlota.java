 ...
 * @since JDK1.6
 */
public class WndNumeroFlota extends WndOpcionesMantenimiento {
	...
	public void onSave(int action) throws Exception {
		try{
			...
			oNumeroFlota.setCodigo(txtCodigo.getText().trim().toUpperCase());
			oNumeroFlota.setNombreCorto(txtNombreCorto.getText().trim().toUpperCase());
			oNumeroFlota.setEstadoRegistro(Constantes.VALUE_ACTIVO);

			switch (action) {
				case ACTION_NEW:
					UtilData.auditarRegistro(oNumeroFlota, getUsuario(), Executions.getCurrent());
					ServiceLocator.getNumeroFlotaManager().guardar(oNumeroFlota);
					textboxId.setText(oNumeroFlota.getId().toString());
					break;

				case ACTION_MODIFY:
					UtilData.auditarRegistro(oNumeroFlota, true, getUsuario(), Executions.getCurrent());
					ServiceLocator.getNumeroFlotaManager().actualizar(oNumeroFlota);
					break;
			}
	...
	public void onDelete(int tab) throws Exception {
		...
		ServiceLocator.getNumeroFlotaManager().inactivar(id);
	}
	...
	private void listarRegistros(ArrayList<NumeroFlota> lstRegistros) {
		...
		for(int r = 0; r < lstRegistros.size(); r ++) {
			...
			lstFila.add(r + 1);
			lstFila.add(oNumeroFlota.getDenominacion());
			lstFila.add(oNumeroFlota.getCodigo());
			lstFila.add(oNumeroFlota.getNombreCorto());

			lstNumeroFlotas.add(lstFila);
		}

		Util.llenarListbox(listboxLista, lstNumeroFlotas, true);
	}


	private void mantenimientoRegistro(Long id) throws Exception {
		oNumeroFlota = ServiceLocator.getNumeroFlotaManager().buscarPorId(id);

		textboxId.setText(oNumeroFlota.getId().toString());
		txtDenominacion.setText(oNumeroFlota.getDenominacion());
		txtCodigo.setText(oNumeroFlota.getCodigo());
		txtNombreCorto.setText(oNumeroFlota.getNombreCorto());
	}
}